...
API_ENDPOINT = ""
CATEGORIES = [
    "http://equipementse3.com/produits-e3/chariot-elevateur/",
]
...
def _process_added_items(items):
    for item in items:
        ...
        try:
            ...
            capacity = ""
        try:
            marque = soup.find("td", class_="Marque : ").text.strip()
        except Exception:
            marque = ""
        try:
            model = soup.find("td", class_="Modèle : ").text.strip()
        except Exception:
            model = ""
        try:
        ...
        try:
            ...
            type_moteur = ""
        try:
            style_pneus = soup.find("td", text="Type de pneu : ").find_next("td").text.strip()
        except Exception:
            style_pneus = ""
        try:
            fourches = soup.find("td", text="Fourche : ").find_next("td").text.strip()
        except Exception:
            fourches = ""
        try:
            attachment = soup.find("td", text="Attachement : ").find_next("td").text.strip()
        except Exception:
            attachment = ""
        data = {
            "post_name": f"{name}",
...
def _crawl_equipementse3_category_2(category_link):
    response_text = request_("GET", category_link).text
    soup = BeautifulSoup(response_text, "html.parser")
    item_links = [
        el.find('a').get('href')
        for el in soup.find_all("div", class_="term-childs")
    ]
    return set(item_links)
...
def crawl_equipementse3(request):
    if request.method == "POST":
        print("[equipementse3] Started crawling website")
        item_links = []
        for category_link in CATEGORIES:
            item_links.extend(
                _crawl_equipementse3_category_2(category_link)
            )
        item_links_subcategory = []
        for category_link in item_links:
            item_links_subcategory.extend(
                _crawl_equipementse3_category(category_link)
            )
        print(f"[equipementse3] Got {len(item_links_subcategory)} item links")

        if not item_links:
            send_warning_email(SENDGRID_API_KEY, SENDER_EMAIL, RECEIVER_EMAILS, "equipementse3")
            return "No links were found on achatusag website"

        db = firestore.Client()

        comparison_result = add_and_compare_new_items(db, "equipementse3", item_links)
        ...
        if email_text != "":
            send_email(SENDGRID_API_KEY, SENDER_EMAIL, RECEIVER_EMAILS, "Comparison results for equipementse3", email_text)
            return email_text
        else:
