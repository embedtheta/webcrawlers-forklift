...
def _parse_item(link):
    print(f"[gregorypoolelift] Processing added link {link}")
...
def _process_added_items(items):
    print(f"[gregorypoolelift] Got {len(items)} added links")
    for item in items:
        print(f"[gregorypoolelift] Posting data about the {item} to forklift.news website")
...
def crawl_gregorypoolelift_pages():
...
def crawl_gregorypoolelift(request):
    if request.method == "POST":
        print("[gregorypoolelift] Started crawling website")
        item_links = crawl_gregorypoolelift_pages()

        db = firestore.Client()

        if not item_links:
            send_warning_email(SENDGRID_API_KEY, SENDER_EMAIL, RECEIVER_EMAILS, "gregorypoolelift")
            return "No links were found on gregorypoolelift website"

        comparison_result = add_and_compare_new_items(db, "gregorypoolelift", item_links)
        ...
        if email_text != "":
            send_email(SENDGRID_API_KEY, SENDER_EMAIL, RECEIVER_EMAILS, "Comparison results for gregorypoolelift",
                       email_text)
            return email_text
        else:
